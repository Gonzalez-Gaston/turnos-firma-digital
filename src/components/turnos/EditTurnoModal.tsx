...
export function EditTurnoModal({ turno, onClose, onUpdate }: EditTurnoModalProps) {
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState({
    fecha: turno.fecha.split('T')[0], // Extract date part
    hora: turno.hora,
    nombre: turno.nombre || '',
    tipodefirma: turno.tipodefirma || '',
    cuil: turno.cuil || '',
    dni: turno.dni || '',
    organismo: turno.organismo || '',
    motivo: turno.motivo || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await turnosApi.updateTurno(turno.id, formData);
      onUpdate();
    } catch (error) {
      console.error('Error updating turno:', error);
      alert('Error al actualizar el turno. Por favor verifica los datos.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            ...
            <p className="text-gray-600 mt-1">ID: {turno.id}</p>
          </div>
          <button
            onClick={onClose}
